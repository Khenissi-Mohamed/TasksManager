...
export default {
    ...
    methods: {
        handleDelete(taskId) {
            axios.delete(`http://task-manager-gtp.up.railway.app/tasks/delete/${taskId}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } });
    ...
    async created() {
        ...
        const tasks = await axios.get("http://task-manager-gtp.up.railway.app/tasks", { headers: { Authorization: `Bearer ${token}` } });
